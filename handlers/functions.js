...
function onCoolDown(message, command) {
    ...
    const cooldownAmount = (command.cooldown || settings.default_cooldown_in_sec) * 2000; //get the cooldownamount of the command, if there is no cooldown there will be automatically 1 sec cooldown, so you cannot spam it^^
  ...
  function GetUser(message, arg){
    var errormessage = "<:Falsee:917802676098588703> I failed finding that User...";
  ...
  function GetRole(message, arg){
    var errormessage = "<:Falsee:917802676098588703> I failed finding that Role...";
    return new Promise(async (resolve, reject) => {
      var args = arg, client = message.client;
      if(!client || !message) return reject("CLIENT IS NOT DEFINED")
      if(!args || args == null || args == undefined) args = message.content.trim().split(/ +/).slice(1);
   ...
   async function swap_pages(client, message, description, TITLE) {
     ...
     collector.on('collect', async b => {
         if(b.user.id !== message.author.id)
           return b.reply(`<:Falsee:917802676098588703> Only the one who typed ${prefix}help is allowed to react!`, true)
           //page forward
           if(b.customId == "1") {
