		...
		try {
			...
			try {
				...
				if (!args[0]) {
					return message.reply({
						embeds: [new MessageEmbed()
							...
							.setDescription(`**Usage:** \`${client.settings.get(message.guild.id, "prefix")}remove <SongPosition> [Amount]\``)
						],
					});
				}
				...
				if (amount <= 0) return message.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setDescription(`You need to at least remove 1 Song!`)
					],
