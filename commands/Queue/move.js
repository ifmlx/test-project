		...
		try {
			...
			try {
				...
				if (!args[0]) {
					return message.reply({
						embeds: [new MessageEmbed()
							...
							.setDescription(`**Usage:** \`${client.settings.get(message.guild.id, "prefix")}move <SongPosition> <ToPosition>\``)
						],
					});
				}
				if (!args[1]) {
					return message.reply({
						embeds: [new MessageEmbed()
							...
							.setDescription(`**Usage:** \`${client.settings.get(message.guild.id, "prefix")}play <SongPosition> <ToPosition>\``)
						],
					});
				}
				...
				if (position == 0) return message.reply({
					embeds: [
						new MessageEmbed().setColor(ee.wrongcolor).setDescription(`Cannot move Song before Playing Song!`)
					],
				...
				message.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setDescription(`📑 Moved **${song.name}** to the **\`${position}th\`** Place right after **${newQueue.songs[position - 1].name}!**`)
				]
				})
			} catch (e) {
