	...
	run: async (client, message, args) => {
		try {
			...
			if (channel.guild.me.voice.channel && channel.guild.me.voice.channel.id != channel.id) {
				return message.reply({
					embeds: [new MessageEmbed()
						...
						.setTitle(`<:Falsee:917802676098588703> I am already connected somewhere else`)
					],
				});
			}
			if (!args[0]) {
				return message.reply({
					embeds: [new MessageEmbed()
						...
						.setDescription(`**Usage:** \`${client.settings.get(message.guild.id, "prefix")}playtop <Search/Link>\``)
					],
				});
			}
			...
			try {
				...
				await client.distube.playVoiceChannel(channel, Text, options).catch(e => {
					console.log(e)
				})
			} catch (e) {
