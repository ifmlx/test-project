...
module.exports = async (client, message) => {
    ...
    if(!cmd || cmd.length == 0){
        if(mPrefix.includes(client.user.id)){
            message.reply(`Hello: **${message.author.username}**, my prefix on this server is \`${prefix}\` Use \`${prefix}help\` to get the list of the commands!`)
        }
        return;
    }
            ...
            if (!botchannels.includes(message.channel.id) && !message.member.permissions.has("ADMINISTRATOR")) {
               return message.reply({
                 ...
                 .setTitle(`${client.allEmojis.x} **You are not allowed to use this Command in here!**`)
                  .setDescription(`Please do it in one of those: ${botchannels.map(c=>`<#${c}>`).join(", ")}`)
                ]
               })
            }
        }
